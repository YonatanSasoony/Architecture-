...
char* get_sh_entry_name(int offset, state* currState){
...
void print_section_names (state* currState){
    ...
    for(int i=0; i<currState->header->e_shnum; i++){
        ...
        printf("[%02d]\t%-*s\t\t%08X\t%06X\t%06X\t%s\n", i,10,get_sh_entry_name(curr->sh_name,currState), curr->sh_addr,curr->sh_offset,curr->sh_size, get_type_name(curr->sh_type));
    }
    if(currState->debug_mode){
...
int get_sh_entry_index_by_name(state* currState,char* entry_name){
    for(int i=0; i<currState->header->e_shnum;i++){
        Elf32_Shdr* curr = (Elf32_Shdr*) (currState->map_start + currState->header->e_shoff + i * currState->header->e_shentsize);
        if(strncmp(get_sh_entry_name(curr->sh_name,currState),entry_name,strlen(entry_name)) == 0)
            return i;
    }
    return -1 ;
}
...
Elf32_Sym* get_dynsym_entry(state* currState, int index){
    for(int i=0; i<currState->header->e_shnum;i++){
        Elf32_Shdr* curr = (Elf32_Shdr*) (currState->map_start + currState->header->e_shoff + i * currState->header->e_shentsize);
        if(curr->sh_type == SHT_DYNSYM)
            return (Elf32_Sym*)(currState->map_start + curr->sh_offset + index * sizeof(Elf32_Sym));
    }
    return NULL ;
}
...
void relocation_tables (state* currState){
    ...
    while((relocation_section_index = get_relocation_section_index(currState,starting_index)) != -1){
        ...
        int size = 0;
        printf("Relocation section '%s' at offset %#X contains %ld entries\n", get_sh_entry_name(relocation_entry->sh_name,currState), offset_relocation_table, relocation_entry->sh_size/sizeof(Elf32_Rel));
        printf("Offset\t\tInfo\t\tType\tSym.value\tSym.Name\n");
        while(size < relocation_entry->sh_size){
            Elf32_Rel* curr = (Elf32_Rel*)(currState->map_start + offset_relocation_table + size);//row in relocation table
            size += sizeof(Elf32_Rel);
            int symbol_index = ELF32_R_SYM(curr->r_info);
            Elf32_Sym* dynsym_entry = get_dynsym_entry(currState, symbol_index);
            int dynstr_index = get_sh_entry_index_by_name(currState,".dynstr");
            Elf32_Shdr* dynstr_entry = (Elf32_Shdr*)(currState->map_start + currState->header->e_shoff + dynstr_index * currState->header->e_shentsize);
            char* name = (char*)(currState->map_start + dynstr_entry->sh_offset + dynsym_entry->st_name);
            printf("%08X\t%08X\t%X\t%08X\t%s\n",curr->r_offset , curr->r_info, ELF32_R_TYPE(curr->r_info), dynsym_entry->st_value, name);
        }
    }
}
